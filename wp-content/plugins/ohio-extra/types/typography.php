<?php
	...
	*/
	...
	function ohio_extra_typography_settings_field( $settings, $value ) {
		...
		switch ( $fonts_type ) {
			case 'adobe_fonts':
				$fonts = OhioExtraAdobeFonts::get_abobe_fonts_array();
				break;
			case 'manual_custom_fonts':
				$fonts = OhioExtraOwnFonts::get_custom_fonts_array();
				break;
			case 'google_fonts':
			default:
				...
				<div class="split-column custom-font-panel"<?php if ( !($value->use_custom_font ?? false) ) echo 'style="display: none;"';?>>
					...
					<div class="input-fonts-wrap">
						<select data-target="custom-font">
							<?php if ($fonts_type == 'google_fonts'){ ?>
								<optgroup label="Recommended for headings">
									<option value="Poppins:600"><?php esc_html_e( 'Poppins', 'ohio-extra' ); ?></option>
									<option value="Montserrat:600"><?php esc_html_e( 'Montserrat', 'ohio-extra' ); ?></option>
									<option value="Playfair Display:700"><?php esc_html_e( 'Playfair Display', 'ohio-extra' ); ?></option>
								</optgroup>
								<optgroup label="Recommended for paragraphs">
									<option value="Open Sans:400"><?php esc_html_e( 'Open Sans', 'ohio-extra' ); ?></option>
									<option value="Roboto:400"><?php esc_html_e( 'Roboto', 'ohio-extra' ); ?></option>
									<option value="Rubik:400"><?php esc_html_e( 'Rubik', 'ohio-extra' ); ?></option>
								</optgroup>
						...
						<?php foreach ($fonts as $font_object) { $_value = $font_object->font_family . ':' . $font_object->font_styles; ?>
							<option value="<?php echo $_value; ?>"<?php if ($_value == ($value->custom_font ?? false)) echo 'selected="selected"'?>><?php echo ucfirst($font_object->font_family); ?></option>
						<?php } ?>
						</select>
					</div>
					<?php if( $fonts_type == 'google_fonts' ) { ?>
