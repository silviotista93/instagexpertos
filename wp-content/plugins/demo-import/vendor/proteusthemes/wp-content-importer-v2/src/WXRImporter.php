...
class WXRImporter extends \WP_Importer {
	...
	public function import( $file ) {
        ...
        $dbgtrace && print('WXRImporter. 1. Filters. ');
		add_filter( 'import_post_meta_key', array( $this, 'is_valid_meta_key' ) );
		add_filter( 'http_request_timeout', array( &$this, 'bump_request_timeout' ) );

        $dbgtrace && print('WXRImporter. 2. Import filte start. ');
        ...
        $dbgtrace && print('WXRImporter. 3. Get reader. ');
        ...
        $dbgtrace && print('WXRImporter. 4. Start parsing. ');
        ...
        $dbgtrace && print('WXRImporter. 5. Post process. ');
		...
		if ( $this->options['aggressive_url_search'] ) {
            $dbgtrace && print('WXRImporter. 5.1. replace_attachment_urls_in_content(). ');
			$this->replace_attachment_urls_in_content();
		}
		// $this->remap_featured_images();

        $dbgtrace && print('WXRImporter. 6.Import end. ');
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( get_post_status( $post['import_id'] ) ) {
