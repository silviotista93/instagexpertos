	...
	public function import_exec( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			if ( @file_exists( $import_file_path ) ) {
				...
				include PT_OCDI_PATH . 'inc/ArgentaDefaultParams.php';
				global $wpdb;
	...
	private function final_response() {
		...
		//if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sDemo Import%4$s plugin, because it has done its job.%5$s', 'pt-ocdi' ),
