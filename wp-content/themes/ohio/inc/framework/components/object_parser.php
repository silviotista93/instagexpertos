...
class OhioObjectParser {
	...
	static function parse_to_post_object( $_post, $_context = 'index', $_index = false ) {
		...
		if ($post_object['media']['blockquote']) {
			...
			if( strpos( $_post->post_content, '<!--more-->' ) ) { 
				$text_before_tag = substr( $_post->post_content, 0, strpos( $_post->post_content, '<!--more-->' ) );
	...
	*/
	static function parse_to_project_object( $_post ) {
		...
		if ( $show_description ) {
			...
			if ( ! get_field( 'project_description', $_post->ID ) ) {
				...
				for ($i = 0; $i < count($_desc_first_space_arr); $i++){
					$project_object['short_description'] .= ' ' . rtrim($_desc_first_space_arr[$i], '.');
					if ($i > $length) break;
				}
        ...
        // set last post as prev
        if ( ! $project_object['prev'] ) {
            $last_post_query = new WP_Query('posts_per_page=1&order=DESC&post_type=ohio_portfolio');

            if ( isset( $last_post_query->posts[0] ) ) {
                $last_post = $last_post_query->posts[0];
                $image = self::get_project_thumbnail_advanced($last_post, $thumbnailSize);
                $project_object['prev'] = array(
                    'title' => $last_post->post_title,
                    'url' => get_permalink($last_post->ID),
                    'image' => $image
                );
            }
		...
		// set first post as next
		if ( ! $project_object['next'] ) {
            $first_post_query = new WP_Query('posts_per_page=1&order=ASC&post_type=ohio_portfolio');

            if ( isset( $first_post_query->posts[0] ) ) {
                $first_post = $first_post_query->posts[0];
                $image = self::get_project_thumbnail_advanced($first_post, $thumbnailSize);
                $project_object['next'] = array(
