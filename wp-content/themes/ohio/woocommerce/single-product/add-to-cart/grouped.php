...
<form class="cart grouped_form woo_c-cart-form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
    <table cellspacing="0" class="woocommerce-grouped-product-list group_table">
        ...
        foreach ( $grouped_products as $grouped_product_child ) {
            ...
            foreach ( $grouped_product_columns as $column_id ) {
                ...
                switch ( $column_id ) {
                    case 'thumb':
                        $value = wp_get_attachment_image( $grouped_product_child->get_image_id());
                        break;
                    case 'quantity':
                        ob_start();
                        echo '<span class="quantity-label">' . esc_html( 'QTY:', 'ohio' ) . '</span>';
                        ...
                        break;
                    case 'label':
                        $value  = '<label class="woo-c_product_name" for="product-' . esc_attr( $grouped_product_child->get_id() ) . '">';
